...
from donatellio.api.types import AssetDisplay, BaseResponse, GetAssetsResponse, GetProjectsResponse, ItemImagePromptChat, JWTToken, RequestCheckElaboratingQuestions, RequestCreateImage, RequestCreateMesh, RequestCreateTexture, RequestCreateUser, RequestEditImage, RequestGetElaboratingQuestions, RequestLoginUser, WSImageEditsResponse, WSImageItem, WSMeshItem, WSMeshResponse
...
from donatellio.utils.hashing import get_password_hash, verify_password
...
async def get_current_user(token: str = Depends(oauth2_scheme), user_dal: UserDAL = Depends(get_user_dal)) -> User:
    """
    Dependency that:
...
async def get_current_user_from_ws(websocket: WebSocket, user_dal: UserDAL = Depends(get_user_dal)):
    """
...
async def authenticate_user(username: str, email: str, password: str, user_dal: UserDAL = Depends(get_user_dal)) -> User:
...
async def register(user: RequestCreateUser, user_dal: UserDAL = Depends(get_user_dal)):
    ...
    refresh_token = create_refresh_token(
        data={"sub": new_user.id, "jti": jti}
    )
    
    return JWTToken(access_token=access_token, token_type="bearer", expires_in=expires_in, refresh_token=refresh_token, success=True)
...
async def login(request: RequestLoginUser, user_dal: UserDAL = Depends(get_user_dal)):
    if request.username is None and request.email is None:
        return JWTToken(success=False, error_msg="Username or email is required", access_token=None, token_type=None, expires_in=None)
    
    db_user = await authenticate_user(request.username, request.email, request.password, user_dal)
    expires_in = datetime.now(timezone.utc) + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    access_token = create_access_token(
        data={"sub": db_user.id}
    )
    ...
    jti = str(uuid.uuid4())
    refresh_token = create_refresh_token(
        data={"sub": db_user.id, "jti": jti}
    )
    
    return JWTToken(access_token=access_token, token_type="bearer", refresh_token=refresh_token, expires_in=expires_in, success=True)

class RequestRefreshToken(BaseModel):
    refresh_token: str

@router.post('/refresh')
async def refresh(request: RequestRefreshToken, user_dal: UserDAL = Depends(get_user_dal)):
    try:
    ...
    return JWTToken(access_token=access_token, refresh_token=request.refresh_token, token_type="bearer", expires_in=expires_in, success=True)

@router.post("/logout", response_model=BaseResponse)
async def logout(request: RequestRefreshToken
