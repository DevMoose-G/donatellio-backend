...
async def generate_mesh(image_id, project_id, mesh_model: str, n_meshes: int, quality: str, seed: int, labels: List[str], max_polygon_count: int) -> List[str]:
    ...
    mesh_ids = await runpod_service.generate_untextured_mesh(project_id, image_id, mesh_model, n_meshes, quality, seed, labels, max_polygon_count)
    ...
    async with AsyncSessionLocal() as session:
        ...
        for mesh_id in mesh_ids:
            ...
            png_storage_key = storage_provider.upload_image(f"{mesh_id}/render.png", render_dict['png'])
            
            convert_dict = run_blender_convert(glb_path, out_dir)
            obj_storage_key = storage_provider.upload_mesh(mesh_id, f"{mesh_id}.obj", convert_dict['obj'])
            fbx_storage_key = storage_provider.upload_mesh(mesh_id, f"{mesh_id}.fbx", convert_dict['fbx'])
            stl_storage_key = storage_provider.upload_mesh(mesh_id, f"{mesh_id}.stl", convert_dict['stl'])
            blend_storage_key = storage_provider.upload_mesh(mesh_id, f"{mesh_id}.blend", convert_dict['blend'])
            
            mesh = await mesh_dal.update_mesh(mesh_id, 
...
async def generate_texture(image_id, project_id, mesh_id: str, prompt: str, texture_quality: str, seed: int) -> str:
    runpod_service = RunpodProvider()
    texture_id = await runpod_service.generate_texture_on_mesh(image_id=image_id, project_id=project_id, mesh_id=mesh_id, prompt=prompt, texture_quality=texture_quality, seed=seed)
    ...
    async with AsyncSessionLocal() as session:
        ...
        render_dict = run_blender_render(glb_path, out_dir)
        png_storage_key = storage_provider.upload_image(f"{texture_id}/render.png", render_dict['png'])
        
        convert_dict = run_blender_convert(glb_path, out_dir)
        obj_storage_key = storage_provider.upload_mesh(texture_id, f"{texture_id}.obj", convert_dict['obj'])
        fbx_storage_key = storage_provider.upload_mesh(texture_id, f"{texture_id}.fbx", convert_dict['fbx'])
        stl_storage_key = storage_provider.upload_mesh(texture_id, f"{texture_id}.stl", convert_dict['stl'])
        blend_storage_key = storage_provider.upload_mesh(texture_id, f"{texture_id}.blend", convert_dict['blend'])
        
        texture = await texture_dal.update_texture(texture_id, 
            static_render_storage_key=png_storage_key, 
            format_storage_keys={
                "obj": obj_storage_key,
                "fbx": fbx_storage_key,
...
async def fill_static_render_images():
    async with AsyncSessionLocal() as session:
        ...
        textures = await texture_dal.get_textures_by(Texture.static_render_storage_key == None)
        for texture in textures:
            ...
            png_storage_key = storage_provider.upload_image(f"{texture.id}/render.png", render_dict['png'])
            await texture_dal.update_texture(texture.id, static_render_storage_key=png_storage_key)
        ...
        for mesh in meshes:
            ...
            png_storage_key = storage_provider.upload_image(f"{mesh.id}/render.png", render_dict['png'])
            await mesh_dal.update_mesh(mesh.id, static_render_storage_key=png_storage_key)
...
async def fill_other_formats():
    async with AsyncSessionLocal() as session:
        ...
        textures = await texture_dal.get_textures_by(Texture.format_storage_keys == None)
        for texture in textures:
            glb_path = f"{TEXTURE_DIR}/{texture.id}.glb"
            storage_provider.download_file(texture.storage_key, glb_path)
            out_dir = f"{TEXTURE_DIR}/{texture.id}"
            ...
            obj_key = storage_provider.upload_mesh(texture.id, f"{texture.id}.obj", convert_dict['obj'])
            fbx_key = storage_provider.upload_mesh(texture.id, f"{texture.id}.fbx", convert_dict['fbx'])
            stl_key = storage_provider.upload_mesh(texture.id, f"{texture.id}.stl", convert_dict['stl'])
            blend_key = storage_provider.upload_mesh(texture.id, f"{texture.id}.blend", convert_dict['blend'])
            
            await texture_dal.update_texture(texture.id, format_storage_keys={
                "obj": obj_key,
                "fbx": fbx_key,
                "stl": stl_key,
                "blend": blend_key
        ...
        for mesh in meshes:
            ...
            out_dir = f"{MESH_DIR}/{mesh.id}"
            convert_dict = run_blender_convert(glb_path, out_dir)
            obj_key = storage_provider.upload_mesh(mesh.id, f"{mesh.id}.obj", convert_dict['obj'])
            fbx_key = storage_provider.upload_mesh(mesh.id, f"{mesh.id}.fbx", convert_dict['fbx'])
            stl_key = storage_provider.upload_mesh(mesh.id, f"{mesh.id}.stl", convert_dict['stl'])
            blend_key = storage_provider.upload_mesh(mesh.id, f"{mesh.id}.blend", convert_dict['blend'])
            
            await mesh_dal.update_mesh(mesh.id, format_storage_keys={
                "obj": obj_key,
                "fbx": fbx_key,
                "stl": stl_key,
                "blend": blend_key
...
def run_blender_convert(glb_path: str, out_dir: str) -> Dict[str, str]:
    ...
    script_path = os.path.join(CURRENT_DIR, "../utils/blender_scripts/convert.py")
...
def run_blender_render(glb_path, out_dir, clear_texture=False) -> Dict[str, str]:
    ...
    script_path = os.path.join(CURRENT_DIR, "../utils/blender_scripts/render.py")
    if not os.path.isfile(script_path):
        raise FileNotFoundError(f"Converter script not found: {script_path}")
