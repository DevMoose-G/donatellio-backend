...
def get_world_bbox(obj):
    ...
    min_corner = mathutils.Vector((min(v.x for v in world_coords),
                                   min(v.y for v in world_coords),
                                   min(v.z for v in world_coords)))
    max_corner = mathutils.Vector((max(v.x for v in world_coords),
...
def main():
    ...
    if "--" not in argv:
        print("Usage: blender --background --python render.py -- <glb_path> <out_dir> <clear_texture>")
        return
