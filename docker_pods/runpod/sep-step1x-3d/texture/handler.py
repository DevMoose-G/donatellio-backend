...
def generate_textured_mesh(request: GenerateTextureRequest, image_path: str, untexture_mesh_path: str):#  input_image_path):
...
def generator_handler(event):
    ...
    mesh = generate_textured_mesh(request, image_path="downloaded_image.png", untexture_mesh_path="downloaded_mesh.glb")
    ...
    print(f"Time taken to generate and upload texture: {time_taken} seconds")
    if response['status_code'] != 200:
        return response
    yield {
