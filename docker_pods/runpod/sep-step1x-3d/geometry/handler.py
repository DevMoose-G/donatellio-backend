...
from torch import set_float32_matmul_precision, FloatTensor, load, save
...
def download_file(url: str, file_loc: str):
    try:
        ...
        if resp.status_code != 200:
            raise Exception(f"Failed to download file from {url}. Response Code: {resp.status_code}. Response: {resp.text}")
...
def generator_handler(event):
    ...
    if request.start_from_latents:
        ...
        for mesh_id, presigned_urls in request.mesh_presigned_urls_mapping.items():
            ...
            upload_asset(f"latents_{i}.pt", latents_presigned_url, "application/octet-stream")
            
            # turn latents into a mesh
            mesh = geom_pipe.postprocess_mesh(latents,
                mc_level=request.mc_level,
                octree_resolution=request.octree_resolution,
                do_remove_degenerate_face=True,
                do_shade_smooth=request.do_shade_smooth,
            ...
            response = upload_asset(mesh_file_loc, mesh_presigned_url, "model/gltf-binary")
        ...
        for mesh_id, presigned_urls in request.mesh_presigned_urls_mapping.items():
            ...
            upload_asset(f"latents_{mesh_id}.pt", latents_presigned_url, "application/octet-stream")
            ...
            mesh = geom_pipe.postprocess_mesh(latents,
                                        ...
                                        do_remove_degenerate_face=True,
                                        do_shade_smooth=request.do_shade_smooth,
                                        max_facenum= request.max_facenum)[0]
