...
class ProjectBranchDAL:
    ...
    async def get_all_branches(self, project_id: str) -> List[ProjectBranch]:
        return await self.session.scalars(select(ProjectBranch).where(ProjectBranch.project_id == project_id))
    
    async def get_branch_by_id(self, branch_id: str) -> ProjectBranch:
        return await self.session.get(ProjectBranch, branch_id)
    
    async def create_branch(self, project_id: str, author_id: str, name: str, version_id: str = None) -> ProjectBranch:
        if version_id is None:
            new_version = await ProjectVersionDAL(self.session).create_version(project_id=project_id, author_id=author_id, version_number=0, message="Initialized branch")
            version_id = new_version.id
        branch = ProjectBranch(id=str(uuid4()), project_id=project_id, name=name, head_version_id=version_id)
        self.session.add(branch)
        await self.session.commit()
        await self.session.refresh(branch)
        return branch
    
    async def update_branch(self, branch_id: str, name: str = None, head_version_id: str = None) -> ProjectBranch:
    ...
    async def create_version(self, branch_id, author_id, version_message) -> ProjectVersion:
        ...
        branch = await self.update_branch(branch_id=branch.id, head_version_id=version.id)
    ...
    async def perform_action(
        ...
        action = await ProjectActionDAL(self.session).create_action(version_id=version.id, author_id=author_id, asset_id=new_asset.id, asset_type=asset_type, action_type=action_type, parameters=parameters)
