...
class ProjectVersionAssetDAL:
    ...
    async def create(
        self, project_version_id: str, asset_type: str, asset_id: str
    ) -> ProjectVersionAsset:
        asset = ProjectVersionAsset(
    ...
    async def get_assets_by_version_id(
        self, project_version_id: str
    ) -> List[ProjectVersionAsset]:
        return await self.session.scalars(
            select(ProjectVersionAsset).where(
                ProjectVersionAsset.project_version_id == project_version_id
            )
        )

    async def seed_from_parent_version(
        self, project_version_id: str, parent_version_id: str
    ) -> List[ProjectVersionAsset]:
        assets: List[ProjectVersionAsset] = await self.get_assets_by_version_id(
            project_version_id=parent_version_id
        )
        for asset in assets:
            await self.create(
                project_version_id=project_version_id,
        ...
        return await self.get_assets_by_version_id(
            project_version_id=project_version_id
        )
    ...
    async def unlink_asset(
        ...
        stmt = select(ProjectVersionAsset).where(
            ProjectVersionAsset.project_version_id == project_version_id,
            ProjectVersionAsset.asset_type == asset_type,
            ProjectVersionAsset.asset_id == asset_id,
        )
    ...
    async def update_from_actions(
        self, project_version_id: str, actions: List[ProjectAction]
    ):
        assets = await self.get_assets_by_version_id(
            project_version_id=project_version_id
        )
        ...
        for action in actions:
            ...
            if action.asset_stage == AssetStage.mesh:
                ...
                if texture is None:
                    raise RuntimeError(
                        400, detail="Texture not found in project version"
                    )
            ...
            if (
                action.asset_id in old_assets.keys()
                and action.asset_stage == old_assets[action.asset_id].asset_type
            ):
                continue
            await self.create(
                project_version_id=project_version_id,
                asset_type=action.asset_stage,
                asset_id=action.asset_id,
            )


def get_project_version_asset_dal(
