...
class ReplicateProvider:
    ...
    async def edit_image(self, image_id: str, model: str, quality: str, prompt: str, original_image_id: str, **kwargs):
        image_name = f"{image_id}.png"
        ...
        if model == "fluxkontext":
            ...
            input_data['aspect_ratio'] = "1:1"
            input_data['safety_tolerance'] = 6 # TEMP: most permissive (TODO: check if this is too low)
        else:
            raise ValueError("Unsupported model")
        
        original_image = await self.dal.image_dal.get_image_by_id(original_image_id)
        original_image_url = self.storage_provider.generate_get_url(original_image.storage_key)
    ...
    async def simplify_mesh(self, mesh_id: str, ratio: float):
        ...
        s3_put_url = self.storage_provider.generate_put_url_for_mesh(f"{mesh_id}_simp_{int(ratio*100)}")
